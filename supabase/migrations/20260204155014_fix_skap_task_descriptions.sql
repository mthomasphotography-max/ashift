/*
  # Fix SKAP Task Description Discrepancies
  
  This migration corrects spelling errors and text discrepancies found when comparing
  the database entries with the source requirements document.
  
  ## Key Corrections
  
  - Fixed spelling errors (e.g., "againt" → "against", "principal" → "principle")
  - Corrected truncated descriptions
  - Aligned wording with source document
  - Fixed consistency issues across similar tasks
*/
...
BEGIN
  ...
  -- Clear and repopulate with corrected descriptions
  DELETE FROM skill_tasks;

  -- New Starter tasks (L1-001 to L1-029) - CORRECTED
  INSERT INTO skill_tasks (skill_level_id, task_name, task_description, sort_order) VALUES
    ...
    (new_starter_id, 'L1-005', 'Knows and understand the basic controls and guidance plus the general principle & purpose of VPO.', 5),
    ...
    (new_starter_id, 'L1-016', 'Understands the basic controls and guidance plus the general principle & purpose of VPO. Awareness of the VPO introduction training document', 16),
    ...
    (new_starter_id, 'L1-023', 'Understands the principle of safe storage, all products and how this can be applied daily.', 23),
    ...
    (new_starter_id, 'L1-026', 'Active against breaks/controls in Logistics by reporting hazards, and be involved in the solution to correct.', 26),
    ...
    (new_starter_id, 'L1-028', 'Know and apply the principles of trailer safety - Safety whilst docker / counterbalance.', 28),
  ...
  -- Beginner tasks (L2-030 to L2-065) - CORRECTED
  INSERT INTO skill_tasks (skill_level_id, task_name, task_description, sort_order) VALUES
    ...
    (beginner_id, 'L2-046', 'Knows and understands how to service a dock-pig (the - removing pallets of out-export loads from the Lines', 46),
    ...
    (beginner_id, 'L2-058', 'Knows and understands the infra safety rules associated with operating within the keg ares in the.', 58),
    ...
    (beginner_id, 'L2-064', 'Understands and apply the Principles of effective VPO implementation in our next link to our PUSH -v with the Excellence programme definitions', 64),
    (beginner_id, 'L2-065', 'EMCS & T1 Export document creation', 65);

  -- Intermediate tasks (L3-066 to L3-075) - CORRECTED
  INSERT INTO skill_tasks (skill_level_id, task_name, task_description, sort_order) VALUES
    ...
    (intermediate_id, 'L3-069', 'Resuming despatch rules and system investigation for wise checking.', 69),
  ...
  -- Advanced tasks (L4-076 to L4-095) - CORRECTED
  INSERT INTO skill_tasks (skill_level_id, task_name, task_description, sort_order) VALUES
    (advanced_id, 'L4-076', 'Understanding and able to instruct others in our processes including supporting the use of early tools and process routines', 76),
    ...
    (advanced_id, 'L4-095', 'Understanding and able to instruct others in our processes including encouraging and supporting the use of early tools and process routines', 95);

  -- Advanced Plus tasks (L4-096 to L4-115) - CORRECTED
  INSERT INTO skill_tasks (skill_level_id, task_name, task_description, sort_order) VALUES
    ...
    (advanced_plus_id, 'L4-115', 'Understanding and able to instruct others in our processes including encouraging and supporting the use of early tools and process routines with a view and Support to AhH', 115);

  -- MOP tasks (L6-116 to L6-131) - CORRECTED
  INSERT INTO skill_tasks (skill_level_id, task_name, task_description, sort_order) VALUES
    (mop_id, 'L6-116', 'Undertake the shift handover when covering for the FLM and ensure that all relevant reports have been updated', 116),
    (mop_id, 'L6-117', 'Receives ar accounts for how the process wor ks per the relevant shift / area within a role potentially responsible for subsequent accident/incident investigation', 117),
    ...
    (mop_id, 'L6-126', 'Carry out Quality Block Stock checks where necessary and disc vigilentials prince as rornsed ay Quality.', 126),
    ...
    (mop_id, 'L6-131', 'Demonstrate shift ownership behaviour, responsibility and ensure food hygiene safety regulations as defined by policy or legislation', 131);
